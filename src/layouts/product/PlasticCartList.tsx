...
import PlasticCartProps from "./componetns/PlasticCartProps";


interface PlasticCartListProps {}

const PlasticCartList: React.FC<PlasticCartListProps> = () => {
    const { setTotalCart, cartList, setCartList } = useCartItem();
    const [totalPriceProduct, setTotalPriceProduct] = useState(0);

    useEffect(() => {
        const total = cartList.reduce((totalPrice, cartItem) => {
            return totalPrice + cartItem.quantity * cartItem.plasticItem.sellPrice;
        }, 0);
    ...
    // Xử lý xoá sách
    function handleRemoveBook(idPlastic: number) {
        const newCartList = cartList.filter(
            (cartItem) => cartItem.plasticItem.idPlasticItem !== idPlastic
        );
                                    ...
                                    {cartList.map((cartItem) => {
                                        return (
                                            <PlasticCartProps
                                                ...
                                                key={cartItem.plasticItem.idPlasticItem}
                                            />
                                        );
                                    })}
                                </div>
...
export default PlasticCartList;
